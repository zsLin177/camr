...
from mrp2tuple import write_tuple_file, normalize_concept_align, get_rels, n_write_tuple_file
...
def ensemble(graph_lst):
    ...
    max_support = 0
    ge_node_tab, ge_edge_tab, ge_attributes = None, None, None
    for i, g_pivot in enumerate(graph_lst):
        other_g_lst = graph_lst[0:i] + graph_lst[i+1:]
        # 'c' means core
        c_node_tab, c_edge_tab, c_attributes, c_relations = initialise(g_pivot, 'c')
        for j, g_curr in enumerate(other_g_lst):
            c_node_tab, c_edge_tab = update_tab(c_node_tab, c_edge_tab, j, g_curr, g_pivot)
        ...
        c_node_tab, c_edge_tab, this_support = filter_tab(c_node_tab, c_edge_tab, top_node_id, threshold)
        if this_support > max_support:
            ge_node_tab, ge_edge_tab, ge_attributes = c_node_tab, c_edge_tab, c_attributes
            max_support = this_support
    
    return ge_node_tab, ge_edge_tab, ge_attributes, max_support
...
def create_ins_rel(node_tab, edge_tab, attributes):
    prefix = 'c'
    ...
    return instance_lst, attributes, rel_lst

def update_tab(c_node_tab, c_edge_tab, curr_g_id, curr_g, g_povit):
    s_prefix = 's' + str(curr_g_id)
    s_node_tab, s_edge_tab, s_attributes, s_relations = initialise(curr_g, s_prefix)
    ...
    return c_node_tab, c_edge_tab

def filter_tab(c_node_tab, c_edge_tab, top_node_id, threshold):
    ...
    return c_node_tab, c_edge_tab, support_sum
...
def initialise(graph, prefix, values={"tops", "labels", "properties", "anchors", "edges", "attributes"}, faith=False):
    for g, _ in mtool.score.core.intersect([graph], []+[graph]):
        ...
        return node_tab, edge_tab, attributes, relations
...
def process(input_dict):
    ...
    ge_node_tab, ge_edge_tab, ge_attributes, max_support = ensemble(graph_lst)
    ge_inst, ge_attr, ge_rel = create_ins_rel(ge_node_tab, ge_edge_tab, ge_attributes)
